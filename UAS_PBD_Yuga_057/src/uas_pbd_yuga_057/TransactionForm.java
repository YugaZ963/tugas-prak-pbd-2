...
public class TransactionForm extends javax.swing.JFrame {
    ...
    try {
        ...
        // Mendapatkan produk yang dipilih dari JComboBox txtId
        String selectedProduct = (String) txtId.getSelectedItem();

        // Memeriksa apakah produk yang dipilih tidak null atau kosong
        if (selectedProduct == null || selectedProduct.isEmpty()) return;

        // Memisahkan ID produk dari string produk yang dipilih dan mengonversinya ke integer
        int productId = Integer.parseInt(selectedProduct.split(" - ")[0]);

        int quantity;
        try {
            // Mengambil teks dari JTextField txtQtyBuy dan mengonversinya ke integer
            quantity = Integer.parseInt(txtQtyBuy.getText());
        } catch (NumberFormatException e) {
            // Jika terjadi kesalahan dalam konversi jumlah, mengatur label lblTotalPrice ke "Total: "
            lblTotalPrice.setText("Total: ");
            return;
        }

        try {
            // Menginisialisasi koneksi ke database dengan memanggil metode configDB dari kelas Config
            con = Config.configDB();

            // Membuat PreparedStatement untuk memilih harga dari tabel product berdasarkan productId
            PreparedStatement pstProduct = con.prepareStatement("SELECT price FROM product WHERE id=?");
            pstProduct.setInt(1, productId); // Mengatur nilai parameter pertama (?) dengan productId

            // Mengeksekusi kueri dan mendapatkan hasilnya
            ResultSet rsProduct = pstProduct.executeQuery();

            // Memeriksa apakah hasil kueri mengembalikan baris
            if (rsProduct.next()) {
                // Mengambil nilai kolom price dari hasil kueri
                double price = rsProduct.getDouble("price");

                // Menghitung total harga dengan mengalikan harga produk dengan jumlah
                double totalPrice = price * quantity;

                // Mengatur teks dari label lblTotalPrice dengan total harga yang dihitung
...
private void loadUserBalance() {
    ...
    // Mengambil teks dari JTextField txtQtyBuy dan mengonversinya ke integer untuk mendapatkan jumlah yang dibeli
    ...
    try {
        ...
        // Menonaktifkan auto-commit untuk memulai transaksi
        con.setAutoCommit(false);

        // Membuat PreparedStatement untuk memilih harga dan kuantitas dari tabel product berdasarkan productId
        PreparedStatement pstProduct = con.prepareStatement("SELECT price, qty FROM product WHERE id=?");
        pstProduct.setInt(1, productId); // Mengatur nilai parameter pertama (?) dengan productId
        ResultSet rsProduct = pstProduct.executeQuery(); // Mengeksekusi kueri dan mendapatkan hasilnya
        
        // Memeriksa apakah hasil kueri mengembalikan baris
        if (!rsProduct.next()) {
            // Menampilkan pesan dialog jika produk tidak ditemukan
            JOptionPane.showMessageDialog(this, "Produk tidak ditemukan");
            return; // Keluar dari metode jika produk tidak ditemukan
        }

        // Mengambil nilai kolom price dan qty dari hasil kueri
        ...
        // Memeriksa apakah jumlah yang dibeli melebihi stok yang tersedia
        if (quantity > availableQuantity) {
            // Menampilkan pesan dialog jika stok tidak mencukupi
            JOptionPane.showMessageDialog(this, "Stok tidak mencukupi");
            return; // Keluar dari metode jika stok tidak mencukupi
        }

        // Membuat PreparedStatement untuk memilih saldo dari tabel akun berdasarkan userId
        PreparedStatement pstUser = con.prepareStatement("SELECT money FROM akun WHERE id=?");
        
        // Mengambil userId dari sesi pengguna saat ini
        int userId = UserSession.userId;
        pstUser.setInt(1, userId); // Mengatur nilai parameter pertama (?) dengan userId
        ResultSet rsUser = pstUser.executeQuery(); // Mengeksekusi kueri dan mendapatkan hasilnya
        
        // Memeriksa apakah hasil kueri mengembalikan baris
        if (!rsUser.next()) {
            // Menampilkan pesan dialog jika pengguna tidak ditemukan
            JOptionPane.showMessageDialog(this, "Pengguna tidak ditemukan");
            return; // Keluar dari metode jika pengguna tidak ditemukan
        }

        // Mengambil nilai kolom money dari hasil kueri
        int money = rsUser.getInt("money");

        // Memeriksa apakah saldo mencukupi untuk total harga pembelian
        if (totalPrice > money) {
            // Menampilkan pesan dialog jika saldo tidak mencukupi
            JOptionPane.showMessageDialog(this, "Saldo tidak mencukupi");
            return; // Keluar dari metode jika saldo tidak mencukupi
        }

        // Membuat PreparedStatement untuk memperbarui kuantitas produk di tabel product
        PreparedStatement pstUpdateProduct = con.prepareStatement("UPDATE product SET qty=? WHERE id=?");
        pstUpdateProduct.setInt(1, availableQuantity - quantity); // Mengurangi stok dengan jumlah yang dibeli
        pstUpdateProduct.setInt(2, productId); // Mengatur nilai parameter kedua (?) dengan productId
        pstUpdateProduct.executeUpdate(); // Mengeksekusi pernyataan SQL untuk memperbarui data

        // Membuat PreparedStatement untuk memperbarui saldo pengguna di tabel akun
        PreparedStatement pstUpdateUser = con.prepareStatement("UPDATE akun SET money=? WHERE id=?");
        pstUpdateUser.setDouble(1, money - totalPrice); // Mengurangi saldo dengan total harga pembelian
        pstUpdateUser.setInt(2, userId); // Mengatur nilai parameter kedua (?) dengan userId
        pstUpdateUser.executeUpdate(); // Mengeksekusi pernyataan SQL untuk memperbarui data

        // Membuat PreparedStatement untuk mencatat transaksi ke tabel transactions
        ...
        pstTransaction.setInt(1, userId); // Mengatur nilai parameter pertama (?) dengan userId
        pstTransaction.setInt(2, productId); // Mengatur nilai parameter kedua (?) dengan productId
        pstTransaction.setInt(3, quantity); // Mengatur nilai parameter ketiga (?) dengan quantity
        pstTransaction.setDouble(4, totalPrice); // Mengatur nilai parameter keempat (?) dengan totalPrice
        pstTransaction.executeUpdate(); // Mengeksekusi pernyataan SQL untuk memasukkan data

        // Mengupdate label lblMoney dengan saldo terbaru
        ...
        // Menampilkan pesan dialog jika pembelian berhasil
        JOptionPane.showMessageDialog(this, "Pembelian berhasil");
    } catch (SQLException e) {
        // Menangani kesalahan jika terjadi kesalahan dalam eksekusi kueri SQL
        e.printStackTrace();
        try {
            // Melakukan rollback pada transaksi jika terjadi kesalahan
            con.rollback();
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    ...
    private void initComponents() {
        ...
        Background1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/uas_pbd_yuga_057/images/gambarToko-1.jpg"))); // NOI18N
